...
func (ma *methodActor) start() {
	reqid := string(ma.req.Reqid)
	replyData, err := processServiceMethod(ma.c, ma.req.Addr, reqid, ma.req.Method, ma.req.Params)
	if err != nil {
	...
	if ma.fromService {
		sendServiceReply(ma.c, ma.req.Addr, reqid, replyData)
	} else {
		sendClientServiceReply(ma.c, ma.req.Addr, reqid, replyData)
	}
}
